...
hexbug.go implements an irrx.RxStateMachine that decodes the IR signals from a 6-button, 4-channel HEXBUG BattleBots remote control.
...
```
	...
	hb := irrx.Hexbug(func(cmd int16) {
		fmt.Printf("%09b\r\n", cmd)
	})
	// create the irrx Rx device; configures rxPin as an input
	rx := irrx.NewRxDevice(rxPin, hb)
...
Here's a simple callback, suitable for passing to irrx.Hexbug(), that takes the first received channel as its own, like how hexbugs themselves work.
...
const (
	HexbugStop          = 0
	HexbugFwdMask       = 0b000000001
	HexbugBackMask      = 0b000000010
	HexbugLeftMask      = 0b000000100
	HexbugRightMask     = 0b000001000
	HexbugRightWeapMask = 0b000010000
	HexbugLeftWeapMask  = 0b000100000

	HexbugButtonMask = HexbugFwdMask | HexbugBackMask | HexbugLeftMask | HexbugRightMask | HexbugLeftWeapMask | HexbugRightWeapMask

	HexbugChMask = 0b011000000
)

const (
	//Hexbug Channel ids; sutiable for comparing with cmd and'ed with a mask: if cmd & HexbugChMask == HexbugCH2
	HexbugCH1 = 0b000000000
	HexbugCH2 = 0b001000000
	HexbugCH3 = 0b011000000 // not a mistake, go figure
	HexbugCH4 = 0b010000000
)
...
func Hexbug(cmdHandler func(int16)) *hexbug {
	return &hexbug{
...
// HandleOnOff implements the RxStateMachine interface
func (hb *hexbug) HandleOnOff(on, off time.Duration) {
